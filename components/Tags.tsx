...
export const Tags: React.FC<Props> = ({...props}) => {
    return <div className={`font-roboto italic ${props.textColour ? props.textColour: 'text-yellow-800'} ${props.backgroundColour ? props.backgroundColour: "bg-yellow-400"} ${props.className} w-fit font-semibold text-[10px] md:text-xs px-4 py-1 rounded-sm`}>{props.children}</div>
}